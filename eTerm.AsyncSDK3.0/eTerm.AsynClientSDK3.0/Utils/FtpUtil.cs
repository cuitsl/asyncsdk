...
namespace eTerm.ASynClientSDK.Utils {
    ...
    public class FtpUtil {
        ...
        /// <summary>
        /// Occurs when [fail authenticate].
        /// </summary>
        public event EventHandler FailAuthenticate;

        /// <summary>
        /// Occurs when [on FTP progress].
        /// </summary>
        public event EventHandler OnFtpProgress;
        ...
        public FtpUtil(string server, string user, string pass)
		{
			this.server = server;
			this.user = user;
			this.pass = pass;
		...
		{
			Disconnect();
            messages = responseStr;
            if (this.FailAuthenticate != null)
                this.FailAuthenticate(this, EventArgs.Empty);
		...
		private void FillBucket()
		{
			...
			while(main_sock.Available < 1)
			{
				...
				{
					Disconnect();
					throw new System.Exception("Timed out waiting on server to respond.");	
				}
			}
		...
		private void OpenDataSocket()
		{
			if (passive_mode)		// #######################################
			{
				...
				{
					Disconnect();
					throw new System.Exception("Malformed PASV response: " + responseStr);
				}
			...
			else		// #######################################
			{
				...
				try
				{
					...
					if (ix < 0)
					{
						throw new System.Exception("Failed to parse the local address: " + sLocAddr);
					}
		...
		private void ConnectDataSocket()		// #######################################
		{
			...
			try
			{
				...
				if (data_sock == null)
				{
					throw new System.Exception("Winsock error: " + 
		...
		{
			if (server == null)
				throw new System.Exception("No server has been set.");
			if (user == null)
				throw new System.Exception("No username has been set.");
			...
			try
			{
                ...
                if (main_sock.Connected&&this.OnConnected!=null ) {
			...
			switch(response)
			{
				case 331:
					...
					{
						Disconnect();
						throw new System.Exception("No password has been set.");
					}
		...
		public string GetWorkingDirectory()
		{
			...
			{
				throw new System.Exception("Uhandled PWD response: " + ex.Message);
			}
        ...
        public void BeginUpload() {
            while (DoUpload() > 0)
                if (this.OnFtpProgress != null)
                    this.OnFtpProgress(this, EventArgs.Empty);
        }
